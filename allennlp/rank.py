"""
The ``rank`` subcommand can be used to
rank a trained model against a dataset
and report any metrics calculated by the model.

.. code-block:: bash

    $ allennlp rank --help
    usage: allennlp rank [-h] [--output-file OUTPUT_FILE]
                             [--weights-file WEIGHTS_FILE]
                             [--cuda-device CUDA_DEVICE] [-o OVERRIDES]
                             [--include-package INCLUDE_PACKAGE]
                             archive_file 

    Rank the specified model + dataset
...
class Rank(Subcommand):
    def add_subparser(self, name: str, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        description = '''Rank the specified model + dataset'''
        subparser = parser.add_parser(
                name, description=description, help='Rank the specified model + dataset')
        ...
        subparser.set_defaults(func=rank_from_args)
...
def rank_from_args(args: argparse.Namespace) -> Dict[str, Any]:
...
#            print("{}  SHAPE {} x {}".format(im,M,N))
            ...
            Q = N/M
            min_sval = np.min(svals)
            print("{} {} {} {} {}".format(im,M,N,Q,min_sval))
